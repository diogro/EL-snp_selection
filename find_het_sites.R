...
snp_types = as.character(unique(just_snps$pu_line)[-2])
...
calculate_snpDensity = function (resolution){
  snp_density_list = dlply(snp_density_binary, .(CHROM), function(x) select(x, A22_hets:A22_het_p_A42))
...
create_snpDesityPlot <- function(resolution, snp_density) {
  ...
  m_snp_density = melt(select(snp_density, CHROM, POS,
                              A22_hets,
                              A23,
                              A22,
                              A42,
