`GAPIT.0000` <-
  function(){
    ...
    GAPIT.Version="2017.08.18, http://zzlab.net/GAPIT"
...
`GAPIT.Bus`<-
  function(Y=NULL,CV=NULL,Z=NULL,KI=NULL,GK=NULL,GD=NULL,GM=NULL,
           ...
           method=NULL,delta=NULL,vg=NULL,ve=NULL,LD=0.01,GTindex=NULL,
           DPP=1000000,method.bin="static",maf.threshold=0.03,bin.size=c(5e5,5e6,5e7),bin.selection=seq(10,100,10),
    ...
    if(method=="Blink")
    {
      ...
      stop("Sorry, I have not Blink into GAPIT now.")
      myFarmCPU=FarmCPU(
        Y=Y,#Phenotype
        GD=GD,#Genotype
        GM=GM,#Map information
...
`GAPIT.DP` <-
  function(Y=NULL,G=NULL,GD=NULL,GM=NULL,KI=NULL,Z=NULL,CV=NULL,CV.Inheritance=NULL,GP=NULL,GK=NULL,
           group.from=30 ,group.to=1000000,group.by=10,DPP=100000, name.of.trait,
           ...
           SNP.MAF=0,FDR.Rate = 1, SNP.FDR=1,SNP.permutation=FALSE,SNP.CV=NULL,SNP.robust="GLM", NJtree.group=4,NJtree.type=c("fan","unrooted"),plot.bin=10^6,PCA.col=NULL,PCA.3d=FALSE,
           ...
           ngrid = 100, llim = -10, ulim = 10, esp = 1e-10,                LD.chromosome=NULL,LD.location=NULL,LD.range=NULL,                sangwich.top=NULL,sangwich.bottom=NULL,QC=TRUE,GTindex=NULL,LD=0.1,
    ...
    Judge=GAPIT.Judge(Y=Y,G=G,GD=GD,KI=KI,GM=GM,group.to=group.to,group.from=group.from,sangwich.top=sangwich.top,sangwich.bottom=sangwich.bottom,kinship.algorithm=kinship.algorithm,PCA.total=PCA.total,model=model,SNP.test=SNP.test)
    #if G exist turn to GD and GM

    myGenotype<-GAPIT.Genotype(G=G,GD=GD,GM=GM,KI=KI,kinship.algorithm=kinship.algorithm,PCA.total=0,SNP.fraction=SNP.fraction,SNP.test=FALSE,
    ...
    return (list(Y=Y,G=G,GD=GD,GM=GI,KI=KI,Z=Z,CV=CV,CV.Inheritance= CV.Inheritance,GP=GP,GK=GK,PC=PC,
                 group.from= Judge$group.from ,group.to= Judge$group.to,group.by= group.by,DPP= DPP, name.of.trait=name.of.trait,
                 ...
                 ngrid = ngrid, llim = llim, ulim = ulim, esp = esp,
                 LD.chromosome= LD.chromosome,LD.location= LD.location,LD.range= LD.range,
                 sangwich.top= sangwich.top,sangwich.bottom= sangwich.bottom,QC= QC,GTindex= GTindex,LD= LD,GT=GT,
                 file.output= file.output,cutOff=0.01, Model.selection = Model.selection,output.numerical = output.numerical,
...
`GAPIT.Genotype` <-
  function(G=NULL,GD=NULL,GM=NULL,KI=NULL,
           kinship.algorithm="Zhang",SNP.effect="Add",SNP.impute="Middle",PCA.total=0,PCA.col=PCA.col,PCA.3d=PCA.3d,seed=123, SNP.fraction =1,
...
`GAPIT.IC` <-
  function(DP=NULL){
    ...
    #print(dim(Y))
    GT=GD[,1]
    if(DP$PCA.total>0 | DP$kinship.algorithm=="Separation"){

      thePCA=GAPIT.PCA(X =as.matrix(GD[,-1]), taxa = GT, PC.number = DP$PCA.total,file.output=DP$file.output,PCA.total=DP$PCA.total,PCA.col=DP$PCA.col,PCA.3d=DP$PCA.3d)
      PC=thePCA$PCs[,1:(1+DP$PCA.total)]
    }
    if(DP$PCA.total>0&!is.null(CV))CV=GAPIT.CVMergePC(CV,PC)
    if(DP$PCA.total>0&is.null(CV))CV=PC
    #print(head(CV))

    #kinship
    if(is.null(DP$KI)&!is.null(DP$kinship.algorithm)&DP$kinship.algorithm!="FarmCPU"&DP$kinship.algorithm!="MLMM"){
      thisGD=GD[,-1]
      myGT=GD[,1]
      if(DP$kinship.algorithm=="EMMA"){
        half.thisGD = as.matrix(.5*thisGD)
        if(length(which(is.na(half.thisGD))) > 0){
          print("Substituting missing values with heterozygote for kinship matrrix calculation....")
          half.thisGD[which(is.na(half.thisGD))] = 1
      ...
      if(DP$kinship.algorithm=="Loiselle")theKin= GAPIT.kinship.loiselle(snps=t(as.matrix(.5*thisGD)), method="additive", use="all")
      if(DP$kinship.algorithm=="VanRaden")theKin= GAPIT.kinship.VanRaden(snps=as.matrix(thisGD))
      if(DP$kinship.algorithm=="Zhang")theKin= GAPIT.kinship.ZHANG(snps=as.matrix(thisGD))
      if(DP$kinship.algorithm=="Separation"&DP$PCA.total>0)theKin= GAPIT.kinship.separation(PCs=thePCA$PCs,EV=thePCA$EV,nPCs=DP$PCA.total)

      colnames(theKin)=myGT
      KI=cbind(myGT,as.data.frame(theKin)) #This require big memory. Need a way to solve it.
      rm(theKin)
      #print(paste("kinship has been built with ",DP$kinship.algorithm," method.",sep=""))
    }else{KI=DP$KI}
    if (is.null(CV))
    {my_allCV=CV
    ...
    return (list(Y=Y,PCA=PCA,K=K,GD=com_GD,GM=DP$GM,my_allCV=my_allCV,my_allGD=my_allGD))
...
`GAPIT.ID` <-
  function(DP=NULL,IC=NULL,SS=NULL,RS=NULL,cutOff=0.01,
    ...
    if(is.null(DP)&is.null(IC))#inputdata is other method result
    {
      ...
      if(!is.null(IC$GM) & !is.null(IC$GD) & DP$file.output & DP$Geno.View.output){
        ViewGenotype<-GAPIT.Genotype.View(
          myGI=IC$GM,
          myGD=IC$GD[,-1],
        )
      ...
      Pred=SS$Pred
      GI=GWAS
      #GI=matrix(as.numeric(as.matrix(GI) ) ,nrow(GI),ncol(GI))
      ##GI=GI[order(GI[,2]),]
      #GI=GI[order(GI[,1]),]
      #GI=GI[order(as.numeric(row.names(GI))),]
      GI=GI[match(as.character(IC$GM[,1]),as.character(GI[,1])),]
      #print(GI[,1:4])
      #print()
      #print(dim(IC$GD))
      #print(dim(DP$GD))
      ...
      if(!is.null(IC$GD)&DP$SNP.test)
      {
        ...
        if(!is.null(PWI.Filtered))
        {
          ...
          PWIP <- GAPIT.Perform.BH.FDR.Multiple.Correction.Procedure(PWI = PWI.Filtered, FDR.Rate = FDR.Rate, FDR.Procedure = "BH")
          print("QQ plot..." )
          if(DP$file.output) GAPIT.QQ(P.values = GWAS$P.value, name.of.trait = DP$name.of.trait,DPP=DP$DPP)
          print("Manhattan plot (Genomewise)..." )
          if(DP$file.output) GAPIT.Manhattan(GI.MP = GI[,c(2:4)], name.of.trait = DP$name.of.trait, DPP=DP$DPP, plot.type = "Genomewise",cutOff=DP$cutOff,seqQTN=DP$QTN.position,plot.style=DP$plot.style,plot.bin=DP$plot.bin)
          #print("@@@@@@@@@@@@@@@@@@@@@@@@")
          print("Manhattan plot (Chromosomewise)..." )
          ...
          print("Creating ROC table and plot" )
          if(DP$file.output) myROC=GAPIT.ROC(t=tvalue,se=stderr,Vp=var(ys),trait=DP$name.of.trait)
          print("ROC table and plot created" )
          ...
          if(DP$file.output){
            write.table(GWAS, paste("GAPIT.", DP$name.of.trait, ".GWAS.Results.csv", sep = ""), quote = FALSE, sep = ",", row.names = FALSE,col.names = TRUE)
      ...
      print(paste(DP$name.of.trait, "has been analyzed successfully!") )
...
`GAPIT.Judge`<-
  function(Y=Y,G=NULL,GD=NULL,KI=NULL,GM=NULL,group.to=group.to,group.from=group.from,sangwich.top=sangwich.top,sangwich.bottom=sangwich.bottom,kinship.algorithm=kinship.algorithm,PCA.total=PCA.total,model="MLM",SNP.test=TRUE){
    ...
    if(!is.null(GD) & is.null(GM) & (is.null(G)) ) stop("GAPIT Says: Genotype data and map files should be in pair")
    if(is.null(GD) & !is.null(GM) & (is.null(G)) ) stop("GAPIT Says: Genotype data and map files should be in pair")
    ...
    if (!is.null(GD))
    {nG=ncol(GD)
    print(paste("There are ",nG," individuals in genotype data."))}
    print("Phenotype and Genotype are test OK !!")

    print("--------------------GAPIT Logical ----------------------------------")
    if (group.to>nY)group.to=nY
    if (group.from>group.to) group.from=group.to
    ...
    if (SUPER_GS)
    {
      ...
      if(hasGenotype &SNP.test &!is.na(Compression[1,4]))     #require not NA REML
      {
        ...
        if(!is.null(PWI.Filtered))
        {
          ...
          print("Manhattan plot (Chromosomewise)..." )
          #  print(cbind(as.data.frame(GT),as.data.frame(GD))[1:20,1:20])
          #if(file.output) GAPIT.Manhattan(GI.MP = PWIP$PWIP[,2:4], name.of.trait = name.of.trait, DPP=DPP, plot.type = "Chromosomewise",cutOff=cutOff)
          if(file.output) GAPIT.Manhattan(GI.MP = cbind(GI[,-1],ps),GD=GD,CG=CG, name.of.trait = name.of.trait, DPP=DPP, plot.type = "Chromosomewise",cutOff=cutOff,plot.bin=plot.bin,chor_taxa=chor_taxa)
      ...
      return (list(Timmer=Timmer,Compression=Compression,kinship.optimum=theK.back, kinship=KI,PC=PC,GWAS=GWAS, GPS=GPS,Pred=Pred,REMLs=Compression[count,4],Timmer=Timmer,Memory=Memory,SUPER_GD=SUPER_GD,P=ps,effect.snp=DTS[,7],effect.cv=p3d$effect.cv,h2= h2.opt))
...
`GAPIT.Mutipl_Manhattan` <-
  function(model_store,DPP=50000,cutOff=0.01,band=5,seqQTN=NULL,Y=NULL,GM=NULL,interQTN=NULL,plot.style="Oceanic",plot.line=FALSE){
    ...
    for(i in 1:length(environ_name))
    {
      ...
      #cutoff=Para$cutOff
      sig_pos=append(sig_pos,as.numeric(rownames(result[result[,4]<(cutOff/nrow(result)),])))
    }
    ...
    lastbase=0
    map_store=GM
    for (i in unique(map_store[,2]))
    {
      index=(map_store[,2]==i)
      map_store[index,3]=as.numeric(as.character(map_store[index,3]))+lastbase
      #print(i)
      #print(head(map_store[index,]))
      lastbase=max(as.numeric(as.character(map_store[index,3])))
      #print(lastbase)
    }
    colnames(x_matrix)=c("pos","times")
    #colnames(xz)=c("pos","col")
    #print(as.numeric(as.character(x_matrix[,1])))
    new_xz=cbind(x_matrix,map_store[as.numeric(as.character(x_matrix[,1])),c(2,3)])
    #print(map_store[as.numeric(x_matrix[,1]),c(2,3)])
    #new_xz[,4]=0
    ...
    pdf(paste("GAPIT.Manhattan.Mutiple.Plot.pdf" ,sep = ""), width = 20,height=10*Nenviron)
    ...
    { if(k==Nenviron){#par(mfrow=c(Nenviron,1))
      ...
      x1=sort(x0)
      #new_xz[,4]=x1[as.numeric(new_xz[,1])]

      position=order(y0,decreasing = TRUE)
      #print(head(x0[position]))
      #print(head(y0[position]))
      #print(x1[as.numeric(new_xz[,1])])
      #index0=GAPIT.Pruning(y0[position],DPP=DPP)
      values=y0[position]
      ...
      mtext(side=2,expression(-log[10](italic(p))),line=3, cex=1.6)
      #Label QTN positions
      #print(head(QTN))
      #print(head(interQTN))
      if(position.only){abline(v=QTN[2], lty = 2, lwd=1.5, col = "grey")}else{
      ...
      if(plot.line){
        ...
        if(!is.null(nrow(new_xz)))  {abline(v=as.numeric(new_xz[,4]),col=plot.color[as.numeric(new_xz[,3])],lty=as.numeric(new_xz[,2]),untf=T,lwd=1.9)
        }else{abline(v=as.numeric(new_xz[1]),col=plot.color[as.numeric(new_xz[3])],lty=as.numeric(new_xz[2]),untf=T,lwd=1.9)
        }
      }
      #Add a horizontal line for bonferroniCutOff
      abline(h=bonferroniCutOff,col="forestgreen")
      axis(2, xaxp=c(1,themax,5),cex.axis=1.8,tick=F)
      if(k==Nenviron)axis(1, at=ticks,cex.axis=1.8,labels=chm.to.analyze,tick=F)
      mtext(side=4,paste(environ_name[k],sep=""),line=3,cex=1.6)
      box()
...
`GAPIT` <-
  function(Y=NULL,G=NULL,GD=NULL,GM=NULL,KI=NULL,Z=NULL,CV=NULL,CV.Inheritance=NULL,GP=NULL,GK=NULL,
           ...
           kinship.cluster="average", kinship.group='Mean',kinship.algorithm="VanRaden",
           bin.from=10000,bin.to=10000,bin.by=10000,inclosure.from=10,inclosure.to=10,inclosure.by=10,
           SNP.P3D=TRUE,SNP.effect="Add",SNP.impute="Middle",PCA.total=0, PCA.col=NULL,PCA.3d=FALSE,SNP.fraction = 1, seed = 123, BINS = 20,SNP.test=TRUE,
...
`GAPIT.SS` <-
  function(DP=NULL,IC=NULL){
    ...
    if(!is.null(IC))
    {
      ...
      if (DP$SNP.test&DP$kinship.algorithm=="FarmCPU"|DP$kinship.algorithm=="MLMM"){
        Timmer=GAPIT.Timmer(Timmer=Timmer,Infor="GAPIT.FarmCPU")
        Memory=GAPIT.Memory(Memory=Memory,Infor="GAPIT.FarmCPU")
        myBus=GAPIT.Bus(Y=ic_Y,CV=ic_PCA,Z=NULL,GK=NULL,KI=ic_KI,GD=ic_GD,GM=ic_GM,
                        ...
                        DPP=DP$DPP,maf.threshold=DP$maf.threshold,file.output=DP$file.output)
        GWAS=myBus$GWAS
        Pred=myBus$GPS
        va=myBus$vg
        ve=myBus$ve
      ...
      if(DP$SNP.test&DP$kinship.algorithm!="FarmCPU"&DP$kinship.algorithm!="MLMM"){
        ...
        gapitMain <- GAPIT.Main(Y=ic_Y,GD=ic_GD[,-1],GM=ic_GM,KI=ic_KI,Z=Z,CV=DP$CV,CV.Inheritance=DP$CV.Inheritance,GP=DP$GP,GK=DP$GK,SNP.P3D=DP$SNP.P3D,kinship.algorithm=DP$kinship.algorithm,
                                ...
                                LD.chromosome=DP$LD.chromosome,LD.location=LD.location,LD.range=LD.range,SNP.CV=SNP.CV,SNP.robust=DP$SNP.robust,
                                genoFormat="EMMA",hasGenotype=TRUE,byFile=FALSE,fullGD=TRUE,PC=ic_PCA,GI=ic_GM,Timmer = DP$Timmer, Memory = DP$Memory,
      ...
      gapitMain <- GAPIT.Main(Y=DP$Y,GD=DP$GD[,-1],GM=DP$GM,KI=DP$KI,Z=DP$Z,CV=DP$CV,CV.Inheritance=DP$CV.Inheritance,GP=DP$GP,GK=DP$GK,SNP.P3D=DP$SNP.P3D,kinship.algorithm=DP$kinship.algorithm,
                              ...
                              LD.chromosome=DP$LD.chromosome,LD.location=LD.location,LD.range=LD.range,SNP.CV=SNP.CV,SNP.robust=DP$SNP.robust,
                              genoFormat="EMMA",hasGenotype=TRUE,byFile=FALSE,fullGD=TRUE,PC=DP$PC,GI=DP$GI,Timmer = DP$Timmer, Memory = DP$Memory,
                              sangwich.top=DP$sangwich.top,sangwich.bottom=DP$sangwich.bottom,QC=DP$QC,GTindex=DP$GTindex,LD=DP$LD,file.output=FALSE,cutOff=DP$cutOff,
                              Model.selection = DP$Model.selection, Create.indicator = DP$Create.indicator,
                              QTN=DP$QTN, QTN.round=DP$QTN.round,QTN.limit=DP$QTN.limit, QTN.update=QTN.update, QTN.method=DP$QTN.method, Major.allele.zero=DP$Major.allele.zero,NJtree.group=DP$NJtree.group,NJtree.type=DP$NJtree.type,plot.bin=DP$plot.bin,
                              QTN.position=DP$QTN.position,plot.style=DP$plot.style,SUPER_GS=DP$SUPER_GS)
      ...
      return (list(GWAS=GWAS,Pred=Pred,FDR=NULL,Power=NULL,
                   Power.Alpha=NULL,alpha=NULL,h2=h2,va=va,ve=ve,
                   mc=mc,bc=bc,mp=mp,
                   Timmer=Timmer,Memory=Memory))
    }#end of SNP.TEST
...
#=============================================================================================
`GAPIT3` <-
  function(Y=NULL,G=NULL,GD=NULL,GM=NULL,KI=NULL,K=NULL,Z=NULL,CV=NULL,CV.Inheritance=NULL,GP=NULL,GK=NULL,
           group.from=1000000 ,group.to=1000000,group.by=10,DPP=100000,
           kinship.cluster="average", kinship.group='Mean',kinship.algorithm="VanRaden",
           bin.from=10000,bin.to=10000,bin.by=10000,inclosure.from=10,inclosure.to=10,inclosure.by=10,
           ...
           WS=c(1e0,1e3,1e4,1e5,1e6,1e7),alpha=c(.01,.05,.1,.2,.3,.4,.5,.6,.7,.8,.9,1),maxOut=100,QTN.position=NULL,CG=NULL,
           converge=1,iteration.output=FALSE,acceleration=0,iteration.method="accum",PCA.View.output=TRUE,Geno.View.output=TRUE,plot.style="Oceanic",SUPER_GD=NULL,SUPER_GS=FALSE,
           h2=NULL,NQTN=NULL,QTNDist="normal",effectunit=1,category=1,r=0.25,cveff=NULL,a2=0,adim=2,
           model="MLM",Para=NULL
  ){
    #Object: To perform GWAS and GPS (Genomic Prediction/Selection)
    #Designed by Zhiwu Zhang
    ...
    echo=TRUE
    #
    if(!is.null(model))if(!match(model,c("MLM","CMLM","SUPER","GLM","FarmCPU","Blink","MLMM"))) stop(paste("PLease choose one model from ","MLM","CMLM","SUPER","GLM","FarmCPU","Blink",sep=""))
    ...
    for(m in 1:length(model_store))
    {
      ...
      if(model=="CMLM")
      {if(is.null(Para$group.from))Para$group.from=50
      if(is.null(Para$group.to))Para$group.to=1000000
      if(is.null(Para$group.by))Para$group.by=50
      }
      if(model=="SUPER")
      {
        if(is.null(Para$inclosure.from))Para$inclosure.from=20
        if(is.null(Para$inclosure.to))Para$inclosure.to=100
        if(is.null(Para$inclosure.by))Para$inclosure.by=20
        if(is.null(Para$bin.from))Para$bin.from=10000
        if(is.null(Para$bin.to))Para$bin.to=50000
      ...
      GAPIT_list=list(group.from=1000000 ,group.to=1000000,group.by=10,DPP=100000,kinship.cluster="average", kinship.group='Mean',kinship.algorithm="VanRaden",
                      bin.from=10000,bin.to=10000,bin.by=10000,inclosure.from=10,inclosure.to=10,inclosure.by=10,SNP.P3D=TRUE,SNP.effect="Add",SNP.impute="Middle",PCA.total=0, SNP.fraction = 1, seed = NULL, BINS = 20,SNP.test=TRUE,
                      SNP.MAF=0,FDR.Rate = 1, SNP.FDR=1,SNP.permutation=FALSE,SNP.CV=NULL,SNP.robust="GLM",file.from=1, file.to=1, file.total=NULL, file.fragment = 99999,file.path=NULL,
                      file.G=NULL, file.Ext.G=NULL,file.GD=NULL, file.GM=NULL, file.Ext.GD=NULL,file.Ext.GM=NULL,ngrid = 100, llim = -10, ulim = 10, esp = 1e-10,
                      LD.chromosome=NULL,LD.location=NULL,LD.range=NULL,PCA.col=NULL,PCA.3d=FALSE,NJtree.group=NULL,NJtree.type=c("fan","unrooted"),
                      sangwich.top=NULL,sangwich.bottom=NULL,QC=TRUE,GTindex=NULL,LD=0.1,plot.bin=10^5,file.output=TRUE,cutOff=0.01, Model.selection = FALSE,output.numerical = FALSE,
                      output.hapmap = FALSE, Create.indicator = FALSE,QTN=NULL, QTN.round=1,QTN.limit=0, QTN.update=TRUE, QTN.method="Penalty", Major.allele.zero = FALSE,
                      method.GLM="fast.lm",method.sub="reward",method.sub.final="reward",method.bin="static",bin.size=c(1000000),bin.selection=c(10,20,50,100,200,500,1000),
                      h2=NULL,NQTN=NULL,QTNDist="normal",effectunit=1,category=1,r=0.25,cveff=NULL,a2=0,adim=2,
                      memo="",Prior=NULL,ncpus=1,maxLoop=3,threshold.output=.01,WS=c(1e0,1e3,1e4,1e5,1e6,1e7),alpha=c(.01,.05,.1,.2,.3,.4,.5,.6,.7,.8,.9,1),maxOut=100,QTN.position=NULL,CG=NULL,
                      converge=1,iteration.output=FALSE,acceleration=0,iteration.method="accum",PCA.View.output=TRUE,Geno.View.output=TRUE,plot.style="Oceanic",SUPER_GD=NULL,SUPER_GS=FALSE)
      ...
      if(!is.null(Y)){
        ...
        for (trait in 2: ncol(Y))  {
          ...
          DP=GAPIT.DP(Y=Y[,c(1,trait)],G=G,GD=GD,GM=GM,KI=KI,Z=Z,CV=CV,CV.Inheritance=Para$CV.Inheritance,GP=GP,GK=GK,
                      group.from=Para$group.from ,group.to= Para$group.to,group.by=Para$group.by,DPP= Para$DPP, name.of.trait=traitname,
                      ...
                      ngrid = Para$ngrid, llim = Para$llim, ulim = Para$ulim, esp = Para$esp,
        ...
        out$GWAS<-SS$GWAS
        out$Pred<-SS$Pred
        if(Para$SNP.test==TRUE)out$QTN<-IC$QTN
        out$Power<-SS$Power
        out$FDR<-SS$FDR
        out$Power.Alpha<-SS$Power.Alpha
        out$alpha<-SS$alpha
        if(Para$SNP.test==TRUE)out$K<-IC$K
        if(Para$SNP.test==TRUE)out$PC<-IC$my_allCV
        out$mc=SS$mc
        out$bc=SS$bc
        out$mp=SS$mp
        out$h2=SS$h2
        ...
        #names(out$GWAS$Para)=c("SNP","Chromosome","Position","mp","mc","maf","nobs")
        ...
        out=c(out,myGenotype)
        out=c(out,myG_simulation)
      } # is.null(Y)
    }#end of model loop
    #print(tail(IC$GM))
    if(!is.null(Y))if(Para$file.output&length(model_store)*(ncol(Y)-1)!=1)
    {
      GAPIT.Mutipl_Manhattan(model_store=model_store,Y=Y,GM=IC$GM)
