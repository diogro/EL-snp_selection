...
#  tx = table(x)
#  if(length(tx) != 2) return(data_frame(V1 = FALSE, V2 = NA))
#  else if(any(names(tx) == ".")) return(data_frame(V1 = FALSE, V2 = NA))
#  else if(any(names(tx) == "*")) return(data_frame(V1 = FALSE, V2 = NA))
#  else return(data_frame(V1 = TRUE, V2 = paste(sort(names(tx)), collapse = "/")))
#}
#
...
#                    .parallel = TRUE))
#just_snps <- just_snps %>%
#     mutate(aleles = is_bialelic$V2,
#            is_bialelic = is_bialelic$V1) %>%
#     select(CHROM, POS, QUAL, TYPE, is_bialelic, aleles,
#             A13_1, A13_2, A13.GQ,
#             A31_1, A31_2, A31.GQ,
#             A41_1, A41_2, A41.GQ,
#             A23_1, A23_2, A23.GQ,
...
#
#not_snps <- not_snps %>%
#     mutate(is_bialelic = FALSE, aleles = NA) %>%
#     select(CHROM, POS, QUAL, TYPE, is_bialelic, aleles,
#             A13_1, A13_2, A13.GQ,
#             A31_1, A31_2, A31.GQ,
#             A41_1, A41_2, A41.GQ,
